    ...
    protected findSymbols(query: string, token: CancellationToken) {
        return new Promise((resolve, reject) => {
            ...
            Crane.langClient.sendRequest(findFileDocumentSymbols, {
                ...
                result.symbols.forEach((item) => {
                    var uri: Uri = this.getUri(item.path);
                    if (uri == null) { return; }
                    var symbol = new SymbolInformation(item.name, item.kind - 1, new Range(item.startLine - 1, item.startChar, item.endLine - 1, item.endChar), uri);
                    results.push(symbol);
                });
                resolve(results);
            });
